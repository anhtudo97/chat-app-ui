...
export interface IUserAPI {
  ...
  updateMe(updateMe: UserUpdate): Promise<Me>;

  isUsernameTaken(searchQuery: string): Promise<boolean>;
...
export class UserAPI implements IUserAPI {
  ...
  constructor(_client: ApolloClient<any>) {
  ...
  async createMe(creation: UserCreation): Promise<Me> {
    ...
    return UserAPI.parserMe(data?.register!);
  }
  ...
  async updateMe(update: UserUpdate): Promise<Me> {
    ...
    return UserAPI.parserMe(data?.updateUser!);
  }

  async getMe(): Promise<Me> {
    const {
      data: { me },
    } = await this._client.query<MeQuery>({
      query: ME_QUERY,
    });

    return UserAPI.parserMe(me);
  }

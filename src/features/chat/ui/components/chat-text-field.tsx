...
export const ChatTextField: React.FC<ChatTextFieldProps> = ({
  ...
  const canChat = useAppSelector((state) => {
    return state.chat.conversations?.find((chat) => chat.id === conversationID)
      ?.canChat;
  });
  ...
  const classes = useStyles({ typing: text.length > 0 || files.length > 0 });
  ...
  const onChange = useCallback((e: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (!timer.current) dispatch(typing({ conversationID, started: true }));
    else clearTimeout(timer.current);

    timer.current = setTimeout(() => {
      dispatch(typing({ conversationID, started: false }));
      timer.current = null;
    }, 3000);
  }, []);
  ...
  const onPaste = useCallback(
    (e: React.ClipboardEvent) => {
      ...
      if (pasted.length) {
        const files: File[] = [];
        for (let i = 0; i < pasted.length; i++) files.push(pasted.item(i)!);
        filesPasted(files);
      }
    },
    [filesPasted]
  ...
  const mediaPreviews = files.map((f, i) => {
    return <MediaPreview file={f} index={i} onClear={fileRemoved} key={i} />;
  });
      ...
      <div className={classes.middleWrapper}>
        ...
        <div className={classes.innerWrapper}>
          ...
          <IconButton
            ...
            onClick={hangleClick}
          >
            <Icon className={classes.buttnIcon}>emoji_emotions</Icon>
          </IconButton>
        </div>
      </div>
      <IconButton

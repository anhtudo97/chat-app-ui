...
export const ChatTextField = (props: ChatTextFieldProps) => {
  ...
  const classes = useStyles({
    typing: text.length > 0 || props.files.length > 0,
  });
  ...
  const onKeyPress: KeyboardEventHandler = useCallback(
    (e) => {
      if (e.code == "Enter" && !e.shiftKey) {
        ...
        submit();
        dispatch(
          typing({ conversationID: props.conversationID, started: false })
        );
  ...
  const mediaPreviews = props.files.map((f, i) => {
    return (
      <MediaPreview file={f} index={i} onClear={props.fileRemoved} key={i} />
    );
  });

...
import { AuthActionContext } from "../../../../src/features/auth/auth-actions-context";
  ...
  return (
    <Provider store={mockStore}>
      <MemoryRouter initialEntries={["/"]} initialIndex={0}>
        <AuthActionContext.Provider value={instance(MockAuthActions)}>
          <RootScreen />
...
test("should render a loading component if the state is not yet initialized", () => {
  // arrange
  const mockStore = MockStore(initialState);
  // act
  renderComponent(mockStore);
...
test("should render the login screen if the state is initialized but no user is logged in", async () => {
  // arrange
...
test("should render the logged in screen if the state is initialized and a user is logged in", async () => {
  // arrange

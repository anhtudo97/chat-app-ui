...
describe("getBadges", () => {
  const act = () => getBadges()(mockStore.dispatch, mockStore.getState, extra);
...
describe("updateBadge", () => {
  const act = (badgeName: BadgeName) =>
    updateBadge(badgeName)(mockStore.dispatch, mockStore.getState, extra);
  ...
  it("should return the right action when rejected", async () => {
    // arrange
    when(MockBadgeRepo.updateBadge(anything())).thenResolve(
      left(BadgeError.network)
    );
    // act
    const result = await act(badgeName);
    // assert

...
>("friends/getFriendRequests", async (_, thunkAPI) => {
...
const declineFriendRequest = createAsyncThunk<
  ...
  string,
  ThunkAPI<FriendError>
>("friends/decline", async (userID, thunkAPI) => {
...
export const _handleRejected = (
  ...
  else state.friendsError = action.payload;
};
...
export const _handleRequestRejected = (
  ...
  const error =
    action.payload === undefined ? FriendError.general : action.payload;
  const userID = action.meta.arg;
  state.requestsError = error;
  state.beingTreated = state.beingTreated.filter((id) => id !== userID);
  if (
    error === FriendError.requestRemoved ||

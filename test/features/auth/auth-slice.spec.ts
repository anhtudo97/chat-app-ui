...
const { signInWithGoogle, getAccessToken, signOut } = authActions;
...
describe("signInWithGoogle thunk", () => {
  const act = () =>
    signInWithGoogle()(mockStore.dispatch, mockStore.getState, extra);

  it("should return the right action when fulfilled", async () => {
    when(MockAuthRepo.signInWithGoogle()).thenResolve(right(loginResult));
  ...
  describe("reducers", () => {
    ...
    it(`should set authUser, accessToken, initialized to true, 
      and loading to false if fulfilled`, async () => {
...
describe("getAccessToken thunk", () => {
  const act = () =>
    getAccessToken()(mockStore.dispatch, mockStore.getState, extra);

  it("should return the right action when fulfilled", async () => {
    // arrange
    when(MockAuthRepo.getAccessToken()).thenResolve(right(accessToken));
    // act
    const result = await act();
    // assert
  ...
  it("should return the right action when rejected", async () => {
    // arrange
    when(MockAuthRepo.getAccessToken()).thenResolve(left(authError));
    // act
    const result = await act();
    // assert
    expect(result.payload).toStrictEqual(authError);
  ...
  describe("reducers", () => {
    ...
    it(`should set the access token and initialized to true, 
    ...
    it(`should set initialized to true, and unset loading if rejected 
...
describe("signOut thunk", () => {
  const act = () => signOut()(mockStore.dispatch, mockStore.getState, extra);

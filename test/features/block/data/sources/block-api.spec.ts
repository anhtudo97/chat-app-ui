...
describe("block", () => {
  it("should block the user and return a block object", async () => {
    ...
    verify(
      MockApolloClient.mutate<BlockMutation, BlockMutationVariables>(
        deepEqual({
          mutation: BLOCK,
          variables: { blockedID },
        })
...
describe("unblock", () => {
  it("should unblock the user and return his/she id", async () => {
    //arrange
    when(MockApolloClient.mutate(anything())).thenResolve({
    ...
    verify(
      MockApolloClient.mutate<Unblock, UnblockVariables>(
        deepEqual({
          mutation: UNBLOCK,
          variables: { blockedID },
        })

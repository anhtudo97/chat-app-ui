...
import { AuthAPI, IAuthAPI } from "./sources/auth-api";
...
export class AuthRepository implements IAuthRepository {
  ...
  constructor(authAPI: IAuthAPI, googleAuth: GoogleAuth) {
    this._authAPI = authAPI;
    this._googleAuth = googleAuth;
  }

  async getAccessToken(): Promise<Either<AuthError, string>> {
  ...
  async signInWithGoogle(): Promise<Either<AuthError, LoginResult>> {
    return this.leftOrRight<LoginResult>(async () => {
      ...
      return this._authAPI.signInWithGoogle(token);
    });
  }
  ...
  async leftOrRight<R>(work: () => Promise<R>): Promise<Either<AuthError, R>> {
    try {
      ...
      if (axios.isAxiosError(e)) {
        if (e.response) {
          if (e.response.status === 401) return left(AuthError.unauthenticated);

...
const useMessageSelector = (convID: number, index: number) => {
  return useAppSelector(
    (state) =>
      state.chat.conversations!.find((c) => c.id === convID)!.messages[index],
    shallowEqual
  );
};
...
type MessageListItemStylesProps = {
  incoming: boolean;
...
const useStyles = makeStyles<Theme, MessageListItemStylesProps>({
  wrapper: (props) => ({
    display: "flex",
...
const MessageLink: React.FC<MessageLinkProps> = ({ href, className }) => {
  return (
...
export const MessageListItem: React.FC<MessageListItemProps> = React.memo(
  ({ index, conversationID, currentUserID }) => {
    ...
    const message = useMessageSelector(conversationID, index);
    const before = useMessageSelector(conversationID, index - 1);
    const after = useMessageSelector(conversationID, index + 1);
    const lastSeen = useAppSelector(
      (state) => state.chat.lastSeen[conversationID],
      shallowEqual
    );
    const incoming = currentUserID !== message.senderID;
    ...
    const showIncomingAvatar =
      incoming && (!after || (after && !_sameSender(message, after)));
    ...
    if (message.text) {
      ...
      if (!spaces) {
        if (isUrl(message.text))
          messageText = (
            <MessageLink href={message.text} className={classes.bubbleType} />
          );
        ...
        words.forEach((word, wordIdx) => {
          ...
          if (isUrl(word)) {
            ...
            currentWord = "";
          } else currentWord = currentWord + word;
        });

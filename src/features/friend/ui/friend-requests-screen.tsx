...
export const FriendRequestsScreen: React.FC<FriendRequestsScreenProps> = ({
  ...
  useEffect(() => {
    if (received) dispatch(updateBadge(BadgeName.FRIEND_REQUESTS));
  }, [received]);
    ...
    (user: User) => {
      ...
      if (history.location.pathname !== pathName)
        history.push({
          pathname: pathName,
          state: {
            viewingUserFromReceivedRequests: received,
            viewingUserFromSentRequests: !received,
            canGoBack: true,
          },
        });
    },
          ...
          {reqs.map((req, index) => {
            ...
            return (
              <RequestListItem
                ...
                onCancel={received ? declineRequest : cancelRequest}
                received={received}
                loading={loading}
                key={index}
              />
  ...
  const leftTopBar = (
    <>
      <BackButton to={received ? "/" : "/requests"} />
      <Typography variant="h6" className={topBarClasses.title}>
        {received ? "Friend requests" : "Sent requests"}
      </Typography>
    </>
  );
  const leftChildren = (

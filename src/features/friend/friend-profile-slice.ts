...
const getFriendshipInfo = createAsyncThunk<
  FriendshipInfo,
  string,
  ThunkAPI<FriendError>
>("friendProfile/getFriendshipInfo", async (username, thunkAPI) => {
  const result = await thunkAPI.extra.friendRepo.getFriendshipInfo({
    username,
  });
...
const sendFriendRequest = createAsyncThunk<
  ...
  void,
  ThunkAPI<FriendError>
>("friendProfile/sendFriendRequest", async (_, thunkAPI) => {
  ...
  const result = await thunkAPI.extra.friendRepo.sendFriendRequest(userID);
  if (isRight(result)) {
...
const cancelFriendRequest = createAsyncThunk<
  ...
  void,
  ThunkAPI<FriendError>
>("friendProfile/cancelFriendRequest", async (_, thunkAPI) => {
  const userID = thunkAPI.getState().friendProfile.user?.id;
  if (!userID) {
    console.log("Cannot cancel a friend request if no user exist in state");
    return thunkAPI.rejectWithValue(FriendError.general);
  }
...
const acceptFriendRequest = createAsyncThunk<
  ...
  void,
  ThunkAPI<FriendError>
>("friendProfile/acceptFriendRequest", async (_, thunkAPI) => {
  const userID = thunkAPI.getState().friendProfile.user?.id;
  if (!userID) {
    console.log("Cannot accecpt a friend request if no user exist in state");
    return thunkAPI.rejectWithValue(FriendError.general);
  }
  const result = await thunkAPI.extra.friendRepo.acceptFriendRequest(userID);
  if (isRight(result)) {
    notifyFriendsSlice(thunkAPI.dispatch, userID);
    return result.right;
  }
  return thunkAPI.rejectWithValue(result.left);
});
...
const declineFriendRequest = createAsyncThunk<
  ...
  void,
  ThunkAPI<FriendError>
>("friendProfile/declineFriendRequest", async (_, thunkAPI) => {
  const userID = thunkAPI.getState().friendProfile.user?.id;
  if (!userID) {
    console.log("Cannot decline a friend request if no user exist in state");
    return thunkAPI.rejectWithValue(FriendError.general);
  }
...
const unfriend = createAsyncThunk<Friendship, void, ThunkAPI<FriendError>>(
  ...
  async (_, thunkAPI) => {
    const userID = thunkAPI.getState().friendProfile.user?.id;
    if (!userID) {
      console.log("Cannot unfriend a friend request if no user exist in state");
      return thunkAPI.rejectWithValue(FriendError.general);
    }

  ...
  return createStyles({
    ...
    textField: (props) => {
      return {
        ...
        "& ::placeholder": {
          /* Most modern browsers support this now. */ color: "black",
          opacity: "0.6",
        },
...
export const SearchTextField: React.FC<SearchTextFieldProps> = ({
  ...
  const onChange = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      ...
      if (text.length) {
        ...
        const newDelay = setTimeout(() => dispatch(searchForUsers(text)), 500);
        setDelay(newDelay);
      }
    },
    [delay]
  ...
  const onFocusEvent: FocusEventHandler = useCallback(() => {
    onFocus();
